...
class QA_constraint(QAGen):
    ...
    def __init__(self):
        ...
        self.description = '''Let C be the curve parametrized by (cos(t), sin(t)) from 0 to pi/2. The density of C at (x, y) is equal to x + y. What is the mass of the curve C using the parametrization with respect to t?'''
    ...
    def Q(s, a, b, x, y, t):
        ...
        given = seqg('Let C be the curve parametrized by (cos({0}), sin({0})) from {1} to {2}. The density of C at ({3}, {4}) is equal to {3} + {4}.'.format(t, a, b, x, y))
        ask = seqg('What is the mass of the curve C using the parametrization with respect to {0}?'.format(t))
        calc = seqg('Find the mass of the curve C using the parametrization with respect to {0}.'.format(t))
        question1 = seqg(given, ask)
        question2 = seqg(given, calc)
    ...
    def A(s, a, b, x, y, t):
        ...
        expression = (x + y) * sqrt(dx**2 + dy**2)
        mass = abs(integrate(expression, (t, a, b)))
        answer1 = seqg('The mass of C is {0}.'.format(mass))
        answer2 = seqg('{0} is the mass of C.'.format(mass))
