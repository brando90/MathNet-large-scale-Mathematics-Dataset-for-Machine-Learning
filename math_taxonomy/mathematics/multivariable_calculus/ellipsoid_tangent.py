...
class QA_constraint(QAGen):
    ...
    def __init__(self):
        ...
        self.description = '''Write the equation of the plane tangent to the ellipsoid 2x^2 + 4y^2 + z^2 = 45 at the point (2, -3, -1).'''
    ...
    def init_qa_variables(self):
        ...
        """
        if self.debug:
            a, b, c, d, e, f = 2, 4, 45, 2, -3, -1
        else:
            a, b, c, d, e, f = np.random.randint(-100, 100, [6])
        return a, b, c, d, e, f

    def Q(s, a, b, c, d, e, f, x, y, z):
        """
        ...
        calc = seqg('Write the equation of the plane tangent to the ellipsoid {0}{1}^2 + {2}{3}^2 + {4}^2 = {5} '
                    'at the point ({6}, {7}, {8}).'.format(a, x, b, y, z, c, d, e, f))
        ask = seqg('What is the equation of the plane tangent to the ellipsoid {0}{1}^2 + {2}{3}^2 + {4}^2 = {5} '
                   'at the point ({6}, {7}, {8})?'.format(a, x, b, y, z, c, d, e, f))
        q = choiceg(calc, ask)
        return q

    def A(s, a, b, c, d, e, f, x, y, z):
        """
        ...
        seqg, perg, choiceg = s.seqg, s.perg, s.choiceg
        function = a*x**2 + b*y**2 + z**2 - c
        gradient = (diff(function, x), diff(function, y), diff(function, z))
        normal = (gradient[0].evalf(subs={x: d, y: e, z: f}), gradient[1].evalf(subs={x: d, y: e, z: f}), gradient[2].evalf(subs={x: d, y: e, z: f}))
        equation = normal[0] * (x - d) + normal[1] * (y - e) + normal[2] * (z - f)

        answer1 = seqg('The equation of the plane tangent to the ellipsoid is {0} = 0.'.format(equation))
        answer2 = seqg('{0} = 0 is the equation of the plane tangent to the ellipsoid.'.format(equation))
