...
class QA_constraint(QAGen):
    ...
    def Q(s,a_val,b_val, c_val, exp, order_val, x, y, z, a, b, c):
        ...
        order = [str(a)+str(x)+str(y)+' + ' + str(b)+str(x) + '^' + str(exp) + ' + ' + str(c) + str(y), str(a)+str(x)+str(y)+' + ' + str(c) + str(y)+ ' + '+ str(b)+str(x) + '^' + str(exp), str(c) + str(y)+ ' + '+ str(b)+str(x) + '^' + str(exp)+ ' + ' + str(a)+str(x)+str(y), str(c) + str(y)+ ' + '+ str(a)+str(x)+str(y)+ ' + '+ str(b)+str(x) + '^' + str(exp), str(b)+str(x) + '^' + str(exp) + ' + ' + str(a)+str(x)+str(y)+' + ' + str(c) + str(y), str(b)+str(x) + '^' + str(exp) + ' + ' + str(c) + str(y)+ ' + '+str(a)+str(x)+str(y)]

        question = seqg('In a certain region of space, the electric potential is', 'V(' + str(x) + ',' + str(y) + ',' + str(z) + ')', '=', str(order[order_val]) + ',', 'where', perg(Eq(a, a_val), Eq(b, b_val), Eq(c, c_val)), '. Find the', str(x) + ', ' + str(y)+ ', and ' + str(z) + ' values', 'where the electric field is zero.')
    ...
    def A(s,a_val,b_val, c_val, exp, order_val, x, y, z, a, b, c):
        ...
        '''
        seqg, perg, choiceg = s.seqg, s.perg, s.choiceg
        expression = a_val*x*y + b_val*(x**exp) + c_val*y
        e_x = -expression.diff(x)
        e_y = -expression.diff(y)
        e_z = -expression.diff(z)
        x_val = solve(e_y, x)
        y_val = (c_val/(b_val*exp))**(1/(exp-1))

        answer1 = seqg('z is any real number.')
        answer2 = seqg('x is', str(x_val) + '.')
        answer3 = seqg('y is', str(y_val) + '.')
