...
class QA_constraint(QAGen):
    ...
    def init_consistent_qa_variables(self):
        ...
        if self.debug:
            l, x, d, z, y, v, j = symbols('l x d z y v j')
        else:
            l,x,d,z, y, v, j = self.get_symbols(7)

        return  l,x,d,z,y, v, j
    ...
    def Q(s, l_val, d_val, diameter_radius, z_val, diameter, l,x,d,z,y, v, j):
        '''
        ...
        terms1 = seqg(perg(l_str + ',', 'Δ'+v_str + ','), 'and ' + z_str)
        terms2 = seqg(perg(z_str + ',', 'Δ'+v_str + ','), 'and', l_str)
        terms3 = seqg(perg(l_str + ',', z_str + ','), 'and Δ' + v_str)
        terms = choiceg(terms1, terms2, terms3)
        ends = choiceg('two ends of the rod', '''rod's two ends''','ends of the rod','two ends')


        d0 = seqg('When a voltage Δ'+v_str+' is applied between the two ends of the rod, what is the current density '+j_str+'? Express your answer in terms of',terms)
        d1 = seqg('When a voltage Δ{0} is applied between the {3}, what is the current density {1}? Express your answer in terms of {2}.'.format(v, j, terms, ends))
        d2 = seqg('{4} the current density {1} between the {3} when a voltage Δ{0} is applied? Express your answer in terms of {2}.'.format(v, j, terms, ends, Whats))
        d3 = seqg('{4} the current density {1} between the {3} when a voltage Δ{0} is applied. Express your answer in terms of {2}.'.format(v, j, terms, ends, Calculate))
        d4 = seqg('Expressing your answer in terms of {2}, {4} the current density {1} between the {3} when a voltage Δ{0} is applied to the rod.'.format(v, j, terms, ends, calculate))
        d5 = seqg('Expressing your answer in terms of {2}, {4} the current density {1} between the {3} when a voltage Δ{0} is applied to the rod?'.format(v, j, terms, ends, whats))
    ...
    def A(s, l_val, d_val, diameter_radius, z_val, diameter, l,x,d,z,y, v, j):
        '''
        Important Note: first variables are the not consistent variables followed
        by the consistent ones. See sample QA example if you need too.
        '''
        ...
        a0 = seqg('{0} is the current density of the {1}.'.format(current_density, rod))
        a1 = seqg('In terms of {0}, the current density of the {1} {2} {3}.'.format(terms, rod, iis, current_density))
        a1 = seqg('''{0} {2} the {1}'s current density.'''.format(current_density, rod,iis))
        a2 = seqg('The current density of the',rod, iis, current_density)
        a3 = seqg('''The {0}'s current density {1} {2}.'''.format(rod, iis, current_density))
        a4 = seqg('{0} is the current density of the {1} in terms of {2}.'.format(current_density, rod, terms))
        a5 = seqg('''{0} is the {1}'s current density when {2}.'''.format(current_density, rod, terms))
        a6 = seqg('The current density of the',rod, iis, current_density,'in terms of',terms)
        a7 = seqg('''The {0}'s current density when {3} {1} {2}'''.format(rod, iis, current_density, terms))
        a8 = seqg('When {2}, {0} is the current density of the {1}.'.format(current_density, rod, terms))
        a9 = seqg('''When {2}, {0} is the {1}'s current density.'''.format(current_density, rod, terms))
