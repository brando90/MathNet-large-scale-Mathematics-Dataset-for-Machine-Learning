...
class QA_constraint(QAGen):
    ...
    def init_consistent_qa_variables(self):
        ...
        if self.debug:
            a, b, l, e, R, I, Q, E, r = symbols('a b l e R I Q E r')
        else:
            a, b, l, e, R, I, Q, E, r = self.get_symbols(9, uppercase = True)

        return a, b, l, e, R, I, Q, E, r
    ...
    def init_qa_variables(self):
        ...
        '''
        if self.debug:
            diameter_radius, diameter, a_val, b_val, l_val, Q_val, R_val = 90, 'radius', 0.1, 0.2, 1, 10, 10
        else:
            ...
            if diameter_radius > 50:
                ...
                diameter = 'diameter'
            a_val = np.random.randint(1,1000)/1000
            b_val = np.random.randint(a_val,2000)/1000
            l_val = np.random.randint(1,999999)/1000
            Q_val = np.random.randint(1,1000)
            R_val = np.random.randint(1, 1000)

        return diameter_radius, diameter, a_val, b_val, l_val, Q_val, R_val

    def Q(s, diameter_radius, diameter, a_val, b_val, l_val, Q_val, R_val,a, b, l, e, R, I, Q, E, r):
        '''
        ...
        Calculate = choiceg('Calculate', 'Compute', 'Find', 'Derive a numerical value for')
        calculate = choiceg('calculate', 'compute', 'find', 'derive a numerical value for')
        ...
        terms1 = seqg(perg(seqg(a,'=',a_val, 'm,'), seqg(b,'=',b_val,'m,'), seqg(l,'=',l_val, 'm,'), seqg(str(r) + ','), seqg(Q,'=',Q_val, 'C,')), 'and', R,'=',R_val,'Ω')
        terms2 = seqg(perg(seqg(R,'=',R_val, 'Ω,'), seqg(b,'=',b_val,'m,'), seqg(l,'=',l_val, 'm,'), seqg(str(r) + ','), seqg(Q,'=',Q_val, 'C,')), 'and', a,'=',a_val,'Ω')
        terms3 = seqg(perg(seqg(a,'=',a_val, 'm,'), seqg(R,'=',R_val, 'Ω,'), seqg(l,'=',l_val, 'm,'), seqg(str(r) + ','), seqg(Q,'=',Q_val, 'C,')), 'and', b,'=',b_val,'Ω')
        terms4 = seqg(perg(seqg(a,'=',a_val, 'm,'), seqg(b,'=',b_val,'m,'), seqg(R,'=',R_val, 'Ω,'), seqg(str(r) + ','), seqg(Q,'=',Q_val, 'C,')), 'and', l,'=',l_val,'Ω')
        terms5 = seqg(perg(seqg(a,'=',a_val, 'm,'), seqg(b,'=',b_val,'m,'), seqg(l,'=',l_val, 'm,'), seqg(str(r) + ','), seqg(R,'=',R_val, 'Ω,')), 'and', Q,'=',Q_val,'Ω')
        terms6 = seqg(perg(seqg(a,'=',a_val, 'm,'), seqg(b,'=',b_val,'m,'), seqg(l,'=',l_val, 'm,'), seqg(R,'=',R_val,'Ω'), seqg(Q,'=',Q_val, 'C,')), 'and', r)

        terms = choiceg(terms1,terms2,terms3,terms4,terms5,terms6)

        GG = seqg(Calculate, 'the magnitude of the electric field',E,'when r < a, r > b, and a < r < b, in terms of',a,'=',a_val ,'m,', b,'=',b_val,'m,', l,'=',l_val, 'm,', str(r) + ',', Q,'=',Q_val, 'C,', 'and', R,'=',R_val, 'Ω.')
        g0 = seqg('{0} the magnitude of the electric field {1} when {2}, in terms of {3}.'.format(Calculate, E,less_than, terms))
        g1 = seqg('When {0}, {1} the magnitude of the electric field {2} in terms of {3}.'.format(less_than, calculate, E, terms))
        g2 = seqg('In terms of {3}, {0} the magnitude of the electric field {1} when {2}.'.format(Calculate, E,less_than, terms))
        g3 = seqg('When {0}, {1} the magnitude of the electric field {2}. Express your answer in terms of {3}.'.format(less_than, calculate, E, terms))
        g4 = seqg('{0} the magnitude of the electric field {1} when {2}.'.format(Calculate, E,less_than))
        g5 = seqg('When {0}, {1} the magnitude of the electric field {2}.'.format(less_than, calculate, E))
    ...
    def A(s, diameter_radius, diameter, a_val, b_val, l_val, Q_val, R_val, a, b, l, e, R, I, Q, E, r):
        '''
        Important Note: first variables are the not consistent variables followed
        by the consistent ones. See sample QA example if you need too.
        '''
        ...
        pi = np.pi
        epsilon_0 = 8.854188 * 10**-12
        zero = choiceg('0', 'zero')

        Eq(E*2*pi*r*l, Q/epsilon_0)
        efield1 = Eq(E, Q_val/(2*pi*r*l_val*epsilon_0))
        efield2 = Eq(Q_val/(2*pi*r*l_val*epsilon_0), E)
        efield = choiceg(efield1, efield2)
        ...
        a0 = seqg('When {0}, the electric field is {1}.'.format(r_a, zero))
        a1 = seqg('The electric field is {1} when {0}.'.format(r_a, zero))
        b0 = seqg('When {0}, the electric field is {1}.'.format(r_b, zero))
        b1 = seqg('The electric field is {1} when {0}.'.format(r_b, zero))
        c0 = seqg('When {0}, the electric field is {1}.'.format(r_a_b, efield))
        c1 = seqg('The electric field is {1} when {0}.'.format(r_a_b, efield))
