...
class QA_constraint(QAGen):
    ...
    def init_consistent_qa_variables(self):
        ...
        if self.debug:
            a, b, l, e, R, I, Q, S, r = symbols('a b l e R I Q S r')
        else:
            a, b, l, e, R, I, Q, S, r = self.get_symbols(9, uppercase = True)

        return a, b, l, e, R, I, Q, S, r
    ...
    def Q(s, diameter_radius, diameter, a, b, l, e, R, I, Q, S, r):
        '''
        ...
        GG = seqg(Calculate, 'the Poynting vector',S,of,'the cable in terms of', str(a) + ',', str(b) + ',', str(l) + ',', str(Q) + ',', str(I) + ',', str(R) + ', and', str(r)+'.')
        g0 = seqg('{0} the Poynting vector {1} {4} {2} in terms of {3}.'.format(Calculate, S,cable, terms, of))
        g1 = seqg('In terms of {3}, {0} the Poynting vector {1} {4} {2}.'.format(calculate, S,cable, terms, of))
        g2 = seqg('{0} the Poynting vector {1} {4} {2}.'.format(Calculate, S, cable, terms, of))
    ...
    def A(s, diameter_radius, diameter, a, b, l, e, R, I, Q, S, r):
        '''
        Important Note: first variables are the not consistent variables followed
        by the consistent ones. See sample QA example if you need too.
        '''
        ...
        poynting1 = Eq(S, (E*B)/mu_0)
        poynting2 = Eq((E*B)/mu_0, S)
        poynting = choiceg(poynting1, poynting2)
        ...
        a0 = seqg('When {0}, the Poynting vector is {1}.'.format(r_a, zero))
        a1 = seqg('The Poynting vector is {1} when {0}.'.format(r_a, zero))
        b0 = seqg('When {0}, the Poynting vector is {1}.'.format(r_b, zero))
        b1 = seqg('The Poynting vector is {1} when {0}.'.format(r_b, zero))
        c0 = seqg('When {0}, the Poynting vector is {1}.'.format(r_a_b, poynting))
        c1 = seqg('The Poynting vector is {1} when {0}.'.format(r_a_b, poynting))
