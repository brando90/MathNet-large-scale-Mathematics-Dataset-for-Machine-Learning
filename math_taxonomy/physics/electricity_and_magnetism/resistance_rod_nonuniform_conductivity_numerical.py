...
class QA_constraint(QAGen):
    ...
    def A(s, l_val, d_val, diameter_radius, z_val, diameter, l,x,d,z,y):
        ...
        l_eq = Eq(l,l_val)
        d_eq = Eq(d, d_val)
        l_d = choiceg(seqg('{0} and {1}'.format(l_eq, d_eq)), seqg('{0} and {1}'.format(d_eq, l_eq)))
        ...
        a4 = seqg('{0} is the resistance of the {1} when {2}.'.format(resistance, rod, l_d))
        a5 = seqg('''{0} is the {1}'s resistance when {2}.'''.format(resistance, rod, l_d))
        a6 = seqg('The resistance of the',rod, iis, resistance,'in terms of',l_d)
        a7 = seqg('''The {0}'s resistance when {3} {1} {2}'''.format(rod, iis, resistance, l_d))
        a8 = seqg('When {2}, {0} is the resistance of the {1}.'.format(resistance, rod, l_d))
        a9 = seqg('''When {2}, {0} is the {1}'s resistance.'''.format(resistance, rod, l_d))
