...
class QA_constraint(QAGen):
    ...
    def init_consistent_qa_variables(self):
        ...
        if self.debug:
            a, b, l, e, R, I, Q, S, r, P = symbols('a b l e R I Q S r P')
        else:
            a, b, l, e, R, I, Q, S, r, P = self.get_symbols(10, uppercase = True)

        return a, b, l, e, R, I, Q, S, r, P
    ...
    def Q(s, diameter_radius, diameter, a, b, l, e, R, I, Q, S, r, P):
        '''
        ...
        cable = choiceg('the cable','both cylinders','the two cylinders','the 2 cylinders','the coaxial cable')
        ...
        terms1 = seqg(perg(str(a) + ',', str(b) + ',', str(l) + ',', str(r) + ',', str(I) + ',', str(Q) + ','), 'and', str(R))
        terms2 = seqg(perg(str(R) + ',', str(b) + ',', str(l) + ',', str(r) + ',', str(I) + ',', str(Q) + ','), 'and', str(a))
        terms3 = seqg(perg(str(a) + ',', str(R) + ',', str(l) + ',', str(r) + ',', str(I) + ',', str(Q) + ','), 'and', str(b))
        terms4 = seqg(perg(str(a) + ',', str(b) + ',', str(R) + ',', str(r) + ',', str(I) + ',', str(Q) + ','), 'and', str(l))
        terms5 = seqg(perg(str(a) + ',', str(b) + ',', str(l) + ',', str(R) + ',', str(I) + ',', str(Q) + ','), 'and', str(r))
        terms6 = seqg(perg(str(a) + ',', str(b) + ',', str(l) + ',', str(r) + ',', str(R) + ',', str(Q) + ','), 'and', str(I))
        terms7 = seqg(perg(str(a) + ',', str(b) + ',', str(l) + ',', str(r) + ',', str(R) + ',', str(I) + ','), 'and', str(Q))
        terms = choiceg(terms1,terms2,terms3,terms4,terms5,terms6, terms7)

        flows = choiceg('that flows into','flowing into','flowing through','that flows through')
        'Calculate the power P that flows into the cable in terms of a, b, l, Q, I, R, and r.'

        GG = seqg(Calculate, 'the power',P,flows,cable,'in terms of', str(a) + ',', str(b) + ',', str(l) + ',', str(Q) + ',', str(I) + ',', str(R) + ', and', str(r)+'.')
        g0 = seqg('{0} the power {1} {4} {2} in terms of {3}.'.format(Calculate, P,cable, terms, flows))
        g1 = seqg('In terms of {3}, {0} the power {1} {4} {2}.'.format(calculate, P,cable, terms, flows))
        g2 = seqg('{0} the power {1} {3} {2}.'.format(Calculate, P, cable, flows))

        g_part = choiceg(GG, g0,g1, g2)

        q = seqg(a_part, b_part, c_d_e, f_part, g_part)

        return q

    def A(s, diameter_radius, diameter, a, b, l, e, R, I, Q, S, r, P):
        '''
        Important Note: first variables are the not consistent variables followed
        by the consistent ones. See sample QA example if you need too.
        '''
        ...
        zero = choiceg('0', 'zero')
        E = Q/(2*pi*r*l*epsilon_0)
        B = (I*mu_0)/(2*pi*r)

        power1 = Eq(P, (E*B*2*pi*(r**2)*ln(b/a))/mu_0)
        power2 = Eq((E*B*2*pi*(r**2)*ln(b/a))/mu_0, P)
        power = choiceg(power1, power2)
        ...
        a0 = seqg('When {0}, the power is {1}.'.format(r_a, zero))
        a1 = seqg('The power is {1} when {0}.'.format(r_a, zero))
        b0 = seqg('When {0}, the power is {1}.'.format(r_b, zero))
        b1 = seqg('The power is {1} when {0}.'.format(r_b, zero))
        c0 = seqg('When {0}, the power is {1}.'.format(r_a_b, power))
        c1 = seqg('The power is {1} when {0}.'.format(r_a_b, power))
