...
class QA_constraint(QAGen):
    ...
    def Q(s, mu_val, m_val, g_val, theta_val, mu, m, g, theta): #TODO change the signature of the function according to your question
        ...
        wanted_V1 = seqg(' For this given {0}, what is the maximum force required to keep the object from falling? The gravitational acceleration is'
                         ' {1} = {2} (m/s^2).'.format(theta, g, g_val))
        wanted_V2 = seqg(' Calculate the maximum force to keep the book up for this given {0}. '
                         'The gravitational acceleration is {1} = {2} (m/s^2).'.format(theta, g, g_val))
        wanted_V3 = seqg(' Based on given information find the maximum force to keep the object up. The gravitational acceleration is'
                         ' {0} = {1} (m/s^2).'.format(g, g_val))
        wanted_V4 = seqg(' Calculate the maximum force to keep the box up for this given {0}. '
                         'The gravitational acceleration is {1} = {2} (m/s^2).'.format(theta, g, g_val))
    ...
    def A(s, mu_val, m_val, g_val, theta_val, mu, m, g, theta): #TODO change the signature of the function according to your answer
        ...
        answer_2 = seqg('The maximum force can be calculated by this equation: {0}')
        answer_3 = seqg('The maximum force is {0} = {1} (N)'.format(F_min, F_min_val))
        explanation_init = seqg('In order to find the maximum force that keep the object on the wall we need to follow'
        ...
        conclusion = seqg(' After solving these three equations we can calculate the maximum force.')
        answer_4 = seqg(explanation_init, perg(explanation_force_x, explanation_force_y, explanation_friction_force),
                        conclusion, 'So the maximum force is ',answer_1)
