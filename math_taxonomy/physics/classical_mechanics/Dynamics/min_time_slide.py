...
class QA_constraint(QAGen):
    ...
    def __init__(self):
        ...
        self.description = 'A block starts at rest and slides down a frictionless plane inclined at angle μ. What should' \
                           ' μ be so that the block travels a given horizontal distance in the minimum amount of time?'
        # keywords about the question that could help to make this more searchable in the future
        self.keywords = ['physics' ,'classical mechanics' ,'block', 'mass', 'Slide', 'Frictionless', 'Plane',
                         'Minimum Time']
        self.use_latex = True
    ...
    def init_consistent_qa_variables(self):
        ...
        """
        if self.debug:
            m, theta, l, g = symbols('m theta l g')
        else:
    ...
    def init_qa_variables(self):
        ...
        return m_val, g_val, l_val

    def Q(s,m_val, g_val, l_val, m, theta, l, g):
        '''
        ...
        info_V1 = seqg('A block with mass {0} = {1} (kg) starts at rest and slides down a frictionless plane inclined '
                       'at angle {2}.'.format(m, m_val, theta))
        info_V2 = seqg('An object with mass {0} = {1} (kg) starts sliding down a frictionless plane inclined at '
                       'angle {2}.'.format(m, m_val, theta))
    ...
    def A(s,m_val, g_val, l_val, m, theta, l, g):
        ...
        seqg, perg, choiceg = s.seqg, s.perg, s.choiceg
        ans_val = 45
        ans_val_radian = 45 * np.pi / 180
        ans_symbol = seqg('{0} = {1} degree'.format(theta, ans_val))
        ans_symbol_rad = seqg('{0} = {1} radian'.format(theta, ans_val))
        g_sin_ = seqg('{0}*sin({1})'.format(g, theta))
        a_x = symbols('a_x')
        a_x_val = seqg('{0}*cos({1})'.format(g_sin_, theta))
        description_V1 = seqg('The component of gravity along the plane in {0}. The acceleration in the horizontal '
