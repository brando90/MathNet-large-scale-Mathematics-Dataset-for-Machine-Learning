...
class QA_constraint(QAGen):
    ...
    def __init__(self):
        ...
        self.description = 'A block of mass m is held motionless on a frictionless plane of mass M and angle of' \
                           ...
                           'The block is released. What is the vertical acceleration of the object?'
    ...
    def init_consistent_qa_variables(self):
        ...
        """
        if self.debug:
            m, M, g, theta, a_y = symbols('m M g theta a_y')
        else:
            m, M, g, a_y = symbols('m M g a_y')
            theta = symbols(chr(952))
        return m, M, g, theta, a_y
    ...
    def Q(s, m_val, M_val, g_val, theta_val, m, M, g, theta, a_y):
        '''
        ...
        wanted_V1 = seqg('What is the vertical acceleration {0} of the object.'.format(a_y))
        wanted_V2 = seqg('Find the vertical acceleration of the object, {0}.'.format(a_y))
        wanted_V3 = seqg('Calculate the vertical acceleration of the object, {0}.'.format(a_y))
        wanted_V4 = seqg('Based on given information, find the vertical acceleration of the object, {0}.'.format(a_y))
     ...
     def A(s, m_val, M_val, g_val, theta_val, m, M, g, theta, a_y):
        '''
        a_y = g * [(cos(theta) )/(cos(theta) + sin(theta)tan(theta)(1 + m/M)) -  1]
        ...
        a_y_val = g_val * (-1 + np.cos(theta_val_rad)/(np.cos(theta_val_rad) + np.sin(theta_val_rad)*np.tan(theta_val_rad)*(1+m_val/M_val)))
        a_y_symbol = seqg('{0} * [(cos({1}/(cos({1}+sin({1})tan({1})(1+{2}/{3})))) - 1)]'.format(g, theta, m, M))
        answer_V1 = seqg('{0} = {1} = {2} (m/s^2)'.format(a_y, a_y_symbol, a_y_val))
        answer_V2 = seqg('The vertical acceleration of the object can be found via this equation ', a_y_symbol, '. Given the values '
                                                                                                    'in the question '
                                                                                                    '{0} = {1} (m/s^2)'
                         .format(a_y, a_y_val))
