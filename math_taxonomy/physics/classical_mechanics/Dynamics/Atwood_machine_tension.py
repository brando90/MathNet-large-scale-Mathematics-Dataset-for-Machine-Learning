...
class QA_constraint(QAGen):
    ...
    def __init__(self):
        ...
        self.description = 'A massless pulley hangs from a fixed support. A massless string connecting two masses,' \
                           ' m1 and m2, hangs over the pulley. Find the tension in the rope?'
    ...
    def Q(s, m1_val, m2_val, g_val, m1, m2, g, T):
        '''
        ...
        info_V2 = seqg('A massless pulley hangs from a fixed support. A massless string connecting two masses,'
                       '0} = {1} (kg) and {2} = {3} (kg), hangs over the pulley.'.format(m1, m1_val, m2, m2_val))
        wanted_V1 = seqg('Find the tension {0} in the string.'.format(T))
        wanted_V2 = seqg('Calculate the acceleration {0} of the mass {1}.'.format(T, m2))
        wanted_V3 = seqg('What is the tension in the string'.format(T))
        wanted_V4 = seqg('Based on given informations what is the tension in the string.'.format(T))
    ...
    def A(s, m1_val, m2_val, g_val, m1, m2, g, T):
        '''
        a1 = 2m1*m2*g/(m1+m2)
        ...
        #define some short cuts
        seqg, perg, choiceg = s.seqg, s.perg, s.choiceg
        T_val = 2*g_val*(m2_val*m1_val)/(m2_val+m1_val)
        T_symbol = seqg('2*{0}*{1}*{2}/({1}+{2})'.format(g, m2, m1))
        answer_V1 = seqg('{0} (m/s^2)'.format(T_val))
        answer_V2 = seqg('{0} = {1} = {2} (m/s^2)'.format(T, T_symbol, T_val))
        answer_V3 = seqg('The tension in the string can be found via the equation {0}. Given the values, '
                         '{1} = {2} (m/s^2).'.format(T_symbol, T, T_val))
